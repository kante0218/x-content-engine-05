...
- アフィ投稿は冒頭に**【PR】**、末尾に「広告リンクを含みます」を付け、X APIにも `paid_partnership: true` を送信
...
- アフィ投稿は既定で**最大1日1件**。残りは育成投稿になる（`X_AFFILIATE_MAX_PER_DAY`で変更可）。
...
- 投稿直前にSHA-256完全一致と文字3-gram近似一致を検査し、重複候補は公開しない。
- 商品リンクは `https://` の有効URLだけを許可する。403/429/5xx時はpendingを保持して次回へ回す。
